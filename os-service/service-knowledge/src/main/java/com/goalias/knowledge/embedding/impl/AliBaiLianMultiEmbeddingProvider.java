...
import com.goalias.knowledge.embedding.MultiModalEmbedModelService;
import com.goalias.knowledge.embedding.model.AliyunMultiModalEmbedRequest;
import com.goalias.knowledge.embedding.model.AliyunMultiModalEmbedResponse;
import com.goalias.knowledge.embedding.model.ModalityType;
import com.goalias.knowledge.embedding.model.MultiModalInput;
import okhttp3.*;
...
public class AliBaiLianMultiEmbeddingProvider implements MultiModalEmbedModelService {
    ...
    public Response<Embedding> embedMultiModal(MultiModalInput input) {
        try {
            ...
            // 构建请求
            AliyunMultiModalEmbedRequest request = buildRequest(contents, chatModel);
            AliyunMultiModalEmbedResponse resp = executeRequest(request, chatModel);
    ...
    public Response<Embedding> embedSingleModality(String key, String dataUrl) {
        try {
            AliyunMultiModalEmbedRequest request = buildRequest(List.of(Map.of(key, dataUrl)), chatModel);
            AliyunMultiModalEmbedResponse resp = executeRequest(request, chatModel);
     ...
     * @param chatModel 模型配置信息
     * @return 构建好的请求对象
     */
    private AliyunMultiModalEmbedRequest buildRequest(List<Map<String, Object>> contents, ChatModel chatModel) {
        if (contents.isEmpty()) throw new IllegalArgumentException("请求内容不能为空");
        return AliyunMultiModalEmbedRequest.create(chatModel.getModelName(), contents);
    }
     ...
     */
    private AliyunMultiModalEmbedResponse executeRequest(AliyunMultiModalEmbedRequest request, ChatModel chatModel) throws IOException {
        String jsonBody = request.toJson();
        RequestBody body = RequestBody.create(jsonBody, MediaType.get("application/json"));

        Request httpRequest = new Request.Builder()
                .url(chatModel.getApiHost())
                .addHeader("Authorization", "Bearer " + chatModel.getApiKey())
                .post(body)
                .build();
